  ...
  <v-data-table :headers="headers" :items="usuarios" :search="search" sort-by="nombreUsuario" class="elevation-1">
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-btn color="primary darken-2" @click="ListadoUsuarioPDF()"><v-icon>print</v-icon></v-btn>
        <v-spacer></v-spacer>
    ...
    <template v-slot:item.actions="{ item }">
      ...
      <template v-if="item.estado">
        <v-icon medium color="green darken-2" class="mr-2" size="x-large"
          @click="modalActivarDesactivar(2, item)">check_circle</v-icon>
      </template>
      <template v-else>
        <v-icon medium color="red darken-2" class="mr-2" size="x-large"
          @click="modalActivarDesactivar(1, item)">cancel</v-icon>
      </template>
  ...
  data: () => ({
    ...
    validar() {
      ...
      if (this.nombreUsuario.length < 3 || this.nombreUsuario.length > 150) //aqui solamente agregué el .lengh en la segunda condicion
        this.ValidaMensajes.push("El nombre del Usuario debe tener más de 3 caracteres y menos de 150");
